...
class Wp_Goal_Tracker_Ga_Admin
{
    ...
    public function __construct($plugin_name, $version)
    {
        ...
        if (gtg_fs()->is__premium_only()) {
            $this->required_options = 'manage_gtga';
        } else {
    ...
    public function add_admin_menu()
    {
        ...
        if (gtg_fs()->is__premium_only()) { # A new premium setting introduced in this version
            ...
            if (!key_exists("generalSettings", $options) || !key_exists("permittedRoles", $options[ "generalSettings" ])) {
                $options[ "generalSettings" ][ "permittedRoles" ] = array(array('id' => 'administrator', 'name' => 'Administrator'));
                update_option('wp_goal_tracker_ga_options', $options);
                Wp_Goal_Tracker_Ga::add_caps($options[ 'generalSettings' ][ 'permittedRoles' ]);
            }
        }
    ...
    public function enqueue_resources()
    {
        ...
        if (gtg_fs()->is__premium_only()) {
            if (gtg_fs()->can_use_premium_code()) {
                $static_path = "apps/pro/build/";
            }
        }

        wp_enqueue_script($this->plugin_name, WP_CUSTOM_EVENTS_TRACKER_URL . $static_path . 'index.js', $dependency, $this->version, true);

        wp_enqueue_style($this->plugin_name, WP_CUSTOM_EVENTS_TRACKER_URL . $static_path . 'index.css', array('wp-components'), $this->version);
        ...
        foreach ($roles as $role_id => $role) {
            array_push($role_names, array('id' => $role_id, 'name' => $role[ 'name' ]));
        }
