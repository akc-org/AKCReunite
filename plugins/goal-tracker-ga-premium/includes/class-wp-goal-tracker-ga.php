...
class Wp_Goal_Tracker_Ga
{
    ...
    private function define_public_hooks()
    {
        ...
        if (gtg_fs()->is__premium_only()) {
            ...
            if (isset($woo_settings)) {
                if (isset($woo_settings[ "viewItem" ]) && $woo_settings[ "viewItem" ]) {
                    $this->loader->add_action('woocommerce_after_single_product', $plugin_public, 'gtga_track_woocommerce_view_item__premium_only');
                }

                if (isset($woo_settings[ "addToCart" ]) && $woo_settings[ "addToCart" ]) {
                    $this->loader->add_action('woocommerce_add_to_cart', $plugin_public, 'gtga_send_ga_add_to_cart_event__premium_only', 10, 6);
                    $this->loader->add_filter("woocommerce_loop_add_to_cart_args", $plugin_public, "gtga_modify_add_to_cart_button__premium_only", 20, 2);
                }

                // if ((isset($woo_settings[ "viewCart" ]) && $woo_settings[ "viewCart" ]) || (isset($woo_settings[ "addShippingInfo" ]) && $woo_settings[ "addShippingInfo" ]) || (isset($woo_settings[ "addPaymentInfo" ]) && $woo_settings[ "addPaymentInfo" ])) {
                //     $this->loader->add_action('woocommerce_after_cart', $plugin_public, 'gtga_track_woocommerce_view_cart__premium_only');
                // }
                if ((isset($woo_settings[ "beginCheckout" ]) && $woo_settings[ "beginCheckout" ]) || (isset($woo_settings[ "addShippingInfo" ]) && $woo_settings[ "addShippingInfo" ]) || (isset($woo_settings[ "addPaymentInfo" ]) && $woo_settings[ "addPaymentInfo" ])) {
                    $this->loader->add_action('woocommerce_after_checkout_form', $plugin_public, 'gtga_track_woocommerce_begin_checkout__primium_only');
                }
                if ($woo_settings[ 'purchase' ] && $woo_settings[ 'purchase' ]) {
                    $this->loader->add_action('woocommerce_thankyou', $plugin_public, 'gtga_send_purchase_event__premium_only');
                }
            }
        }
    ...
    public function db_upgrade()
    {
        ...
        $options = get_option('wp_goal_tracker_ga_options');
        if ($options === false) {
            $options = array(); // Initialize as an empty array if the option does not exist
        }
        ...
        if (!isset($options[ "db_version" ])) {
            ...
            if (gtg_fs()->is__premium_only()) { # A new premium setting introduced in this version
                $general_settings[ "permittedRoles" ] = array(array('id' => 'administrator', 'name' => 'Administrator'));
                Wp_Goal_Tracker_Ga::add_caps($general_settings[ "permittedRoles" ]);
            }
