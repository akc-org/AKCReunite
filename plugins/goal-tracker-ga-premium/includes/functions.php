    ...
    function wp_goal_tracker_ga_default_options($key = '')
{
        ...
        $default_general_settings = array(
            ...
            'trackLinks' => array('enabled' => false, 'type' => "all"),
            'trackEmailLinks' => false,
            'pageSpeedTracking' => false,
        ...
        if (gtg_fs()->is__premium_only()) {
            $default_plugin_options[ "generalSettings" ][ "permittedRoles" ] = array(array('id' => 'administrator', 'name' => 'Administrator'));
            $default_plugin_options[ "videoSettings" ] = array('gaYoutubeVideoTracking' => false, 'gaVimeoVideoTracking' => false, 'gaMediaVideoTracking' => false, 'gaMediaAudioTracking' => false);
            $default_plugin_options[ "formTrackingSettings" ] = array('contactForm7Settings' => array('trackFormSubmit' => false, 'trackInvalids' => false, 'trackMailSent' => false, 'trackMailFailed' => false, 'trackSpam' => false));
            $default_plugin_options[ "ecommerceTrackingSettings" ] = array('wooCommerceSettings' => array('viewItem' => false, 'addToCart' => false, 'viewCart' => false, 'beginCheckout', 'addShippingInfo' => false, 'addPaymentInfo' => false, 'purchase' => false));
            $default_plugin_options[ "generalSettings" ][ "trackUsers" ] = false;
        }
    ...
    function wp_goal_tracker_ga_update_option($option_key, $option)
{
        ...
        if (!$updated) {
            return new WP_Error('error_updating_options', 'Failed to update options.', [ 'status' => 500 ]);
        }
    }
endif;
    ...
    function wp_goal_tracker_ga_set_general_settings($new_general_settings)
{
        ...
        foreach ($new_general_settings as $key => $value) {
            ...
            if (!$has_administrator) {
                array_push($value, array('id' => 'administrator', 'name' => 'Administrator'));
            }
    ...
    function wp_goal_tracker_ga_set_video_settings($new_video_settings)
{
        ...
        foreach ($new_video_settings as $key => $value) { # sanitize values - all are boolean
            $value = (bool) $value;
    ...
    function wp_goal_tracker_ga_set_form_tracking_settings($new_form_tracking_settings)
{
        ...
        foreach ($new_form_tracking_settings as $key => $value) { # sanitize values - all are boolean
    ...
    function wp_goal_tracker_ga_set_ecommerce_tracking_settings($new_ecommerce_tracking_settings)
{
        ...
        $form_tracking_settings = array();
        foreach ($new_ecommerce_tracking_settings as $key => $value) { # sanitize values - all are boolean
...
{
        if (isset($ID) && is_int($ID)) // get specific post config
    {
            return get_post_meta($ID);
        } else { // return all posts of given types
            $all_events_data = [  ];
            ...
            foreach ($all_events as &$event) {
                ...
                $event_meta[ "props" ] = get_post_meta($event, "props", true); // workaround to unserialize issue
                $event_meta[ "id" ] = $event;
    ...
    function wp_goal_tracker_ga_set_config($type, $ID, $config)
{
        ...
        $ID = wp_insert_post($postarr, true); // Note the 'true' to return WP_Error on failure

        // Check if wp_insert_post returned a WP_Error object
        if (is_wp_error($ID)) {
            // You can return the WP_Error object directly or create a new one
            return new WP_Error('error_inserting_post', 'Failed to create event post: ' . $ID->get_error_message(), [ 'status' => 500 ]);
        }
...
{
        if (isset($ID)) // get specific post config
    {
    ...
    function wp_goal_tracker_ga_set_entire_config($config = '')
{
        ...
        if (gtg_fs()->is__premium_only()) {
            wp_goal_tracker_ga_update_option("videoSettings", $options[ "videoSettings" ]);
        }
    ...
    function wp_goal_tracker_ga_update_cache_settings()
{
        ...
        if ($updated === false) {
            ...
            return new WP_Error('error_updating_options', 'Failed to update options. It might be due to the new value being identical to the existing one or a database error.', [ 'status' => 500 ]);
        }
    }
endif;